...
CdWpAllCosmoStudy::CdWpAllCosmoStudy(const std::string& name) : 
    Analysis{name}
{}

bool CdWpAllCosmoStudy::initialize() {
...
void CdWpAllCosmoStudy::process(JM::NavBuffer* buf) {
    ...
    for (std::vector<std::vector<track>>::const_iterator it = tracks.begin(); it != tracks.end(); ++it) {
        ...
        for (std::vector<track>::const_iterator jt = it->begin(); jt != it->end(); ++jt) {
            ...
            if (jt->quality != -1.0f) {
                ...
                mu_cosmo_cut.emplace_back(*jt, 40000.0, TimeStamp{0, 5000000}, TimeStamp{0, 1200000000});
            }
            else {
