...
public:
    ...
    cosmo_shape_analysis(const std::string& filepath, const std::string& suffix, const timestamp& low, const timestamp& high, double radius) :
    ...
    bool selection() override {
        ...
        for (std::size_t k = 0ul; k < m_nav->method_mu.size() && !found; ++k) {
            ...
            m_dlat_mu2p = d_mu2p;
            m_dlat_mu2d = d_mu2d;
            m_dt_mu2p = timestamp_to_double(m_nav->prompt.ts - ts_mu);
            m_dt_mu2d = timestamp_to_double(m_nav->delayed.ts - ts_mu);
        }
    ...
    bool process() override {
        m_cosmos.insert({m_nav->prompt, m_nav->delayed, m_dlat_mu2p, m_dlat_mu2d, m_dt_mu2p, m_dt_mu2d});
        return true;
    }

    void result() override {
        std::vector<cosmogenic> cosmos;
        cosmos.reserve(m_cosmos.size());
        for (std::set<cosmogenic>::const_iterator it = m_cosmos.begin(); it != m_cosmos.end(); ++it) {
            cosmos.push_back(*it);
        }
        ...
        for (const cosmogenic& v : cosmos) { // cosmos_before
            h_e_p_cosmo_before->Fill(v.prompt.e);
            h_e_d_cosmo_before->Fill(v.delayed.e);
            h_dt_cosmo_before->Fill(timestamp_to_double(v.delayed.ts - v.prompt.ts) * 1000.0);
            h_dr_cosmo_before->Fill(mag(v.delayed.pos - v.prompt.pos) / 1000.0);
            h_rho_z_p_cosmo_before->Fill((v.prompt.pos.x * v.prompt.pos.x + v.prompt.pos.y * v.prompt.pos.y) / 1.0e6, v.prompt.pos.z / 1000.0);
            h_rho_z_d_cosmo_before->Fill((v.delayed.pos.x * v.delayed.pos.x + v.delayed.pos.y * v.delayed.pos.y) / 1.0e6, v.delayed.pos.z / 1000.0);
        }

        for (const cosmogenic& v : cosmos) { // cosmos_after
            h_e_p_cosmo_after->Fill(v.prompt.e);
            h_e_d_cosmo_after->Fill(v.delayed.e);
            h_dt_cosmo_after->Fill(timestamp_to_double(v.delayed.ts - v.prompt.ts) * 1000.0);
            h_dr_cosmo_after->Fill(mag(v.delayed.pos - v.prompt.pos) / 1000.0);
            h_rho_z_p_cosmo_after->Fill((v.prompt.pos.x * v.prompt.pos.x + v.prompt.pos.y * v.prompt.pos.y) / 1.0e6, v.prompt.pos.z / 1000.0);
            h_rho_z_d_cosmo_after->Fill((v.delayed.pos.x * v.delayed.pos.x + v.delayed.pos.y * v.delayed.pos.y) / 1.0e6, v.delayed.pos.z / 1000.0);
        }
